...
//used by the constructor in 'app.component.ts'
export class MessageService {
  ...
  constructor() {
    ...
    this.socket.on('user connected', (data) => {
      this._chats.push({message: `${data} joined the chat`, type: 'notify'});
      this._chatssub.next([...this._chats]);
    });
    this.socket.on('user disconnected', (data) => {
      this._chats.push({message: `${data} left the chat`, type: 'notify'});
      this._chatssub.next([...this._chats]);
    });
  }
  addChat(message) {
